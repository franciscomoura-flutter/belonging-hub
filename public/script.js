function showMain(id) {
    ...
    // Journey catalog enhancement
    if (id === 'journey') {
        const catRoot = document.getElementById('journeyCatalog');
        if (catRoot) enhanceCatalogFromMarkup(catRoot);
    }
...
// Journey Catalog Enhancer
function enhanceCatalogFromMarkup(root, options = {}) {
    if (!root || root.dataset.enhanced === 'true') return;

    const source = root.querySelector('.catalog-source');
    if (!source) return;

    const normRole = (r) => {
        const s = String(r || '').trim().toLowerCase();
        if (s.includes('hiring')) return 'hiring-manager';
    ...
    const articles = [...source.querySelectorAll('article')];
    const items = articles.map((el, i) => {
        const title = el.dataset.title || el.getAttribute('data-title') || el.querySelector('h4')?.textContent || `Item ${i + 1}`;
        const description = el.dataset.description || el.getAttribute('data-description') || el.textContent.trim();
        const link = el.dataset.link || el.getAttribute('data-link') || '#';
        const category = (el.dataset.category || el.getAttribute('data-category') || 'General').trim();
        const role = normRole(el.dataset.role || el.getAttribute('data-role') || 'everyone');
        const thumbnail = el.dataset.thumbnail || el.getAttribute('data-thumbnail') || '';
        return { title, description, link, category, role, thumbnail };
    }).filter(x => x.title && x.link);

    if (!items.length) return;

    // Unique roles with counts
    const roleCounts = items.reduce((acc, it) => {
        acc[it.role] = (acc[it.role] || 0) + 1;
        return acc;
    }, {});

    const roleOrder = ['everyone', 'manager', 'hiring-manager'];
    const roles = roleOrder.filter(r => roleCounts[r]).concat(
        Object.keys(roleCounts).filter(r => !roleOrder.includes(r))
    );

    // Build controls and content
    const controls = document.createElement('div');
    controls.className = 'catalog-controls';
    const roleFilter = document.createElement('div');
    roleFilter.className = 'role-filter';
    roleFilter.setAttribute('role', 'tablist');
    roles.forEach((r, idx) => {
        ...
        btn.dataset.role = r;
        btn.textContent = `${roleLabel(r)} (${roleCounts[r]})`;
        btn.addEventListener('click', () => selectRole(r));
    ...
    // Hide source block (kept in DOM for content management)
    source.style.display = 'none';

    // Render logic
    function selectRole(role) {
        root.dataset.selectedRole = role;
        // Update active button
        roleFilter.querySelectorAll('button').forEach(b => {
    ...
    function renderRole(role) {
        ...
        // Group by category
        const byCat = roleItems.reduce((acc, it) => {
        ...
        groupsWrap.className = 'catalog-groups';
        ...
        cats.forEach(cat => {
            ...
            grid.className = 'catalog-items';

            byCat[cat].forEach(item => grid.appendChild(makeCard(item)));
    ...
    function makeCard(item) {
        ...
        if (item.thumbnail) {
            ...
            // Lightweight preview: favicon based on link domain
            try {
                const u = new URL(item.link);
                img.src = `https://www.google.com/s2/favicons?sz=128&domain_url=${encodeURIComponent(u.origin)}`;
    ...
    // Default role selection (prefer Everyone, then Manager, then Hiring Managers)
    const defaultRole = roles[0];
    selectRole(root.dataset.selectedRole || defaultRole);
}
